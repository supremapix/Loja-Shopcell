...
export default function ProductCard({ product, onSelect, onAddToCart }: ProductCardProps) {
  ...
  return (
    <motion.div
      ...
      className="bg-white border border-slate-200 rounded-2xl overflow-hidden hover:border-[#FF6600]/40 transition-all duration-300 flex flex-col h-full shadow-sm hover:shadow-lg group relative"
    >
      {/* Product Image and badges */}
      <div className="relative pt-[100%] bg-slate-50/60 overflow-hidden flex items-center justify-center p-6">
        {/* Category Badge */}
        <div className="absolute top-3 left-3 flex flex-wrap gap-1.5 z-10">
          <span className="bg-white text-[#FF6600] border border-[#FF6600]/25 font-mono text-[9px] font-bold px-2.5 py-0.5 rounded-md uppercase tracking-wider shadow-xs">
            {product.brand}
          </span>
            ...
            <span
              ...
              className={`font-mono text-[9px] font-bold px-2 py-0.5 rounded-md uppercase tracking-wider ${
                b.includes('Novo') || b.includes('🔥') || b.includes('NOVO')
                  ...
                  : 'bg-slate-100 text-gray-700 border border-slate-200/50'
              }`}
            >
              {b}
            </span>
      ...
      <div className="p-4 flex flex-col flex-grow">
        ...
        {/* Title */}
        <h3 className="font-display font-extrabold text-gray-900 text-sm sm:text-base leading-snug line-clamp-2 mb-1.5 group-hover:text-[#FF6600] transition-colors duration-200">
          {product.name}
        </h3>

        {/* Description */}
        <p className="text-gray-500 text-xs line-clamp-2 mb-4 leading-relaxed flex-grow">
          {product.desc}
        </p>

        {/* Pricing */}
        <div className="border-t border-slate-100 pt-3 mt-auto flex flex-col mb-4">
          <div className="flex items-baseline gap-2">
            ...
            <span className="text-base font-display font-black text-gray-900">
              {product.priceAt.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}
            </span>
          </div>
          <div className="flex justify-between items-center mt-1">
            <span className="text-[9px] text-gray-400 uppercase font-mono tracking-wider">À vista no PIX</span>
        ...
        <div className="grid grid-cols-2 gap-2 mt-auto">
          ...
          <button
            ...
            className="flex items-center justify-center gap-1.5 bg-slate-50 hover:bg-slate-100 border border-slate-200 text-gray-700 text-xs font-bold py-2.5 px-3 rounded-xl transition-all duration-300 uppercase tracking-wider cursor-pointer"
        ...
        <a
          ...
          target="_blank"
          referrerPolicy="no-referrer"
          className="block text-center text-[10px] text-gray-500 hover:text-[#FF6600] font-mono uppercase tracking-wider mt-3 transition-colors"
